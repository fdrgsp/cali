...
def save_trace_data_to_csv(
    ...
    cali_logger.info(f"Exporting Traces Data to `{path}`...")
    try:
        _export_raw_data(path, analysis_data)
    except Exception as e:
        cali_logger.error(f"Error exporting RAW DATA to CSV: {e}")
    try:
        _export_corrected_data(path, analysis_data)
    except Exception as e:
        cali_logger.error(f"Error exporting CORRECTED DATA to CSV: {e}")
    try:
        _export_dff_data(path, analysis_data)
    except Exception as e:
        cali_logger.error(f"Error exporting dFF DATA to CSV: {e}")
    try:
        _export_dec_dff_data(path, analysis_data)
    except Exception as e:
        cali_logger.error(f"Error exporting DEC_DFF DATA to CSV: {e}")
    try:
        _export_inferred_spikes_data(path, analysis_data)
    except Exception as e:
        cali_logger.error(f"Error exporting INFERRED RAW SPIKES DATA to CSV: {e}")
    try:
...
def save_analysis_data_to_csv(
    ...
    try:
        ...
        cali_logger.error(f"Error exporting spontanoous analysis data to CSV: {e}")
    try:
        _export_to_csv_mean_values_evk_parameters(path, rearrange_cond_evk)
    except Exception as e:
        cali_logger.error(f"Error exporting evoked analysis data to CSV: {e}")
    cali_logger.info("Exporting Analysis Data to CSV: DONE!")
...
def _rearrange_by_parameter_evk(
    ...
    if parameter == AMP_STIMULATED_PEAKS:
        try:
            ...
            cali_logger.error(f"Error calculating stimulated peaks: {e}")
            return {}
    ...
    if parameter == AMP_NON_STIMULATED_PEAKS:
        try:
            ...
            cali_logger.error(f"Error calculating non-stimulated peaks: {e}")
            return {}
    return {}
