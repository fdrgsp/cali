...
class _AnalyseCalciumTraces(QWidget):
    ...
    def run(self) -> None:
        ...
        cali_logger.info("Number of positions: %s", len(pos))
    ...
    def update_widget_form_settings(self) -> None:
        ...
        if not settings_json_file.exists():
            cali_logger.warning(f"Settings file {settings_json_file} not found")
            return None
    ...
    def _prepare_for_running(self) -> list[int] | None:
        ...
        if not self._validate_input_data():
            cali_logger.error("Input data validation failed!")
            return None

        if not (analysis_path := self._get_valid_output_path()):
            cali_logger.error("Output path validation failed!")
            return None
    ...
    def _prepare_stimulation_mask(self, analysis_path: Path) -> bool:
        ...
        if stim_area_file := value.experiment_type_data.stimulation_area_path:
            ...
            cali_logger.info("Stimulated Area Mask saved at: %s", analysis_path)
            return True
    ...
    def _get_positions_to_analyze(self) -> list[int] | None:
        ...
        cali_logger.info("Positions to analyze: %s", positions)
        return positions
    ...
    def _extract_traces_data(self, positions: list[int]) -> Generator[str, None, None]:
        ...
        cali_logger.info("Threads: %s", threads)
        ...
        try:
            with ThreadPoolExecutor(max_workers=threads) as executor:
                ...
                for idx, future in enumerate(as_completed(futures)):
                    ...
                    try:
                        future.result()
                        cali_logger.info(f"Position {positions[idx]} completed.")
            ...
            cali_logger.info("All positions processed.")
    ...
    def _handle_plate_map(self) -> None:
        ...
        # save plate map
        cali_logger.info("Saving Plate Maps.")
    ...
    def _on_worker_finished(self) -> None:
        ...
        cali_logger.info("Traces Analysis Finished.")
    ...
    def _on_worker_errored(self) -> None:
        ...
        cali_logger.info("Extraction of traces terminated with an error.")
        self._cleanup_after_completion()
    ...
    def _save_analysis_data(self, pos_name: str) -> None:
        ...
        cali_logger.info("Saving JSON file for Well %s.", pos_name)
        if not self._analysis_path:
            return
    ...
    def _save_settings_as_json(self) -> None:
        ...
        try:
            ...
            cali_logger.error(f"Failed to save noise multiplier: {e}")
        # fmt: on
