...
4. **Baseline correction (shift predictor)**: To distinguish true functional connectivity from slow co-modulations (e.g., both neurons increasing activity over time), we compute a null model using circular shifts of one spike train. The baseline is computed from 50 shuffled surrogates:
