...
from cali.sqlmodel._model import FOV, FOVAnalysis, CaliResult
...
with Session(engine) as session:
    ...
    if fov_analysis.calcium_population_activity:
        ...
        print(f"Calcium population activity range: [{pop_activity.min():.4f}, {pop_activity.max():.4f}]")
    ...
    if fov.rois:
        ...
        if rois_with_traces:
            ...
            if roi.data_analysis_history:
                ...
                if data_analysis.total_recording_time_sec:
                    print(f"  Recording time from data_analysis: {data_analysis.total_recording_time_sec:.2f} s")
        ...
        if calcium_traces:
            ...
            if rois_rec_time:
                ...
                print(f"Recording time range: [{min(rois_rec_time):.2f}, {max_rec_time:.2f}] s")
                ...
                print(f"  Should be around 200 seconds!")
            else:
