...
def test_combo_disabled_without_fov_or_run(
    ...
    # All plots should be disabled (added 2 CCG z-score plots)
    assert _count_combo_items(widget, enabled=False) == 48
...
def test_combo_disabled_with_only_run_id(
    ...
    # (68 plots: spontaneous + evoked + sorted rising edges CCG + 2 CCG z-score plots
    #  + 2 sorted max lag values + 2 sorted z-score for evoked
    #  + sorted spike synchrony rising edges)
    assert _count_combo_items(widget, enabled=False) == 68
