...
def test_calcium_peaks_synchrony_emits_roi_selected(
    qtbot: QtBot, test_matrix: np.ndarray, test_roi_labels: list[int]
) -> None:
    """Test calcium peaks synchrony plot emits roiSelected on click."""
    from cali.plot._single_wells_plots.correlation._plot_calcium_peaks_synchrony import (  # noqa: E501
    ...
    with patch(
        "cali.plot._single_wells_plots.correlation._plot_calcium_peaks_synchrony."
        "_get_synchrony_matrix_from_db"
    ) as mock_get:
        mock_get.return_value = (
            test_matrix,
            test_roi_labels,
            0.5,  # global_synchrony
            200.0,  # jitter_window_ms
        )
        ...
        # Should emit signal with ROIs at position (0, 1)
        assert len(emitted_rois) == 1
        # Point(0, 1) = col=0, row=1 -> roi_labels[1], roi_labels[0]
        assert emitted_rois[0] == ["10", "5"]
...
def test_multiple_clicks_emit_multiple_signals(
    qtbot: QtBot, test_matrix: np.ndarray, test_roi_labels: list[int]
) -> None:
    ...
    with patch(
        ...
        mock_get.return_value = (test_matrix, test_roi_labels, 0.5, 200.0)

        _plot_peak_event_synchrony_data(
            widget=widget, engine=mock_engine, fov_name="test", rois=None, run_id=1
        )
        ...
        plot = widget.plot_item
        assert plot is not None
        handler = plot.property("sync_click_handler")

        vb = plot.getViewBox()

        # Click twice at different positions
        positions = [(0, 0), (1, 1)]

        for row, col in positions:
            with patch.object(vb, "mapSceneToView") as mock_map:
                mock_map.return_value = pg.Point(col, row)
        ...
        # Should have two emissions
        assert len(emitted) == 2
        assert emitted[0] == ["5", "5"]  # (0, 0) -> both roi_labels[0]
        assert emitted[1] == ["10", "10"]  # (1, 1) -> both roi_labels[1]
